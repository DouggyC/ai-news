...
export default function ComparePage() {
    ...
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-[#0f2744] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-zinc-500 dark:text-zinc-400">Loading model data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#0f2744]">
      <main className="max-w-[98vw] mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-12">
          ...
          <p className="text-lg text-zinc-600 dark:text-zinc-300 max-w-3xl">
            Compare the latest flagship AI models across pricing, capabilities, parameters, and benchmarks. Use the interactive column headers to sort the table.
          </p>
        </div>
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead>
                <tr className="bg-zinc-50 dark:bg-zinc-800/50 text-sm border-b border-zinc-200 dark:border-zinc-800/50">
                  ...
                  <th className="py-4 px-6 font-semibold text-zinc-900 dark:text-zinc-200 text-right cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors" onClick={() => handleSort('inputPrice')}>
                    In ($/M) {sortField === 'inputPrice' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th className="py-4 px-6 font-semibold text-zinc-900 dark:text-zinc-200 text-right cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors" onClick={() => handleSort('outputPrice')}>
                    Out ($/M) {sortField === 'outputPrice' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>

                  <th className="py-4 px-6 font-semibold text-zinc-900 dark:text-zinc-200 text-right cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors" onClick={() => handleSort('liveBench')}>
                    LiveBench {sortField === 'liveBench' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th className="py-4 px-6 font-semibold text-zinc-900 dark:text-zinc-200 text-center cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors" onClick={() => handleSort('multimodal')}>
                    Multi {sortField === 'multimodal' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th className="py-4 px-6 font-semibold text-zinc-900 dark:text-zinc-200 text-center cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors" onClick={() => handleSort('vision')}>
                    Vision {sortField === 'vision' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th className="py-4 px-6 font-semibold text-zinc-900 dark:text-zinc-200 text-center cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors" onClick={() => handleSort('functionCalling')}>
                    Func {sortField === 'functionCalling' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th className="py-4 px-6 font-semibold text-zinc-900 dark:text-zinc-200 text-center cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors" onClick={() => handleSort('search')}>
                    Search {sortField === 'search' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                </tr>
              </thead>
                ...
                {sortedModels.map((model) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 text-right">
                      <span className={`font-medium ${model.free ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-700 dark:text-zinc-300'}`}>
                    ...
                    <td className="py-4 px-6 text-center text-lg">{getFeatureIcon(model.multimodal)}</td>
                    <td className="py-4 px-6 text-center text-lg">{getFeatureIcon(model.vision)}</td>
                    <td className="py-4 px-6 text-center text-lg">{getFeatureIcon(model.functionCalling)}</td>
                    <td className="py-4 px-6 text-center text-lg">{getFeatureIcon(model.search)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 bg-white dark:bg-[#153457] rounded-2xl border border-zinc-200 dark:border-zinc-800/50 p-8 shadow-xl shadow-blue-900/5">
          <div>
            ...
            <ul className="space-y-3 text-sm text-zinc-600 dark:text-zinc-400">
              <li className="flex items-center gap-2">
                <span className="px-2 py-1 bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 rounded font-medium border border-emerald-200 dark:border-emerald-500/20">Free</span>
                <span>Open weights/No API cost</span>
              </li>
              <li className="flex items-center gap-2">
                <strong className="text-zinc-900 dark:text-zinc-100">$X.XX</strong>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-zinc-600 dark:text-zinc-400">
              <li className="flex items-center gap-2">
                <span className="px-2 py-1 bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 rounded font-medium border border-emerald-200 dark:border-emerald-500/20">Open Source</span>
                <span>Available for download</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="px-2 py-1 bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400 rounded font-medium border border-indigo-200 dark:border-indigo-500/20">Proprietary</span>
          ...
          <div className="col-span-1 lg:col-span-2">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-zinc-600 dark:text-zinc-400">
              <div className="flex items-center gap-2"><span className="text-lg">✅</span> Supported out-of-the-box</div>
              <div className="flex items-center gap-2"><span className="text-lg">❌</span> Lacking capability</div>
              <div><strong>Multi:</strong> Natively Multimodal architecture</div>
              <div><strong>Vision:</strong> Vision/Image input allowed</div>
              <div><strong>Func:</strong> Native Function Calling support</div>
              <div><strong>Search:</strong> Web Search grounding supported</div>
            </div>
          </div>
        </div>
